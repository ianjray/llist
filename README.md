...
int main(void)
{
    ...
    ((struct node *)list_at(list_insert(list_end(l), make(0))))->a = 2;
    list_insert(list_end(l), make(3));
    assert(list_size(l) == 2);

    // Or prepended.
    // Insert before the first element (list_begin).
    n = list_at(list_insert(list_begin(l), make(1)));
    ...
    list_erase((struct list_iter *)(&n->link), free);
    assert(list_size(l) == 4);

    n = list_at(list_begin(l));
    n = list_at(list_advance((struct list_iter *)(&n->link), 1));
    assert(n->a == 2);
